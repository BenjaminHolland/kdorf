package land.generic.kdorf.parsing
...
open class ArgParser(
     ...
     */
    private fun printError(message: String) = outputAndTerminate("$message\n${makeUsage()}", 127)
     ...
     */
    private suspend fun recognizeAndSaveOptionFullForm(candidate: String, argIterator: Iterator<String>): Boolean {
     ...
     */
    internal suspend fun saveOptionWithoutParameter(argValue: ParsingValue<*, *>) {
        // Boolean flags.
     ...
     * @param argIterator iterator over command line arguments.
     */
    private suspend fun saveStandardOptionForm(argValue: ParsingValue<*, *>, argIterator: Iterator<String>) {
     ...
     * @param argIterator iterator over command line arguments.
     */
    private suspend fun recognizeAndSaveOptionShortForm(candidate: String, argIterator: Iterator<String>): Boolean {
     ...
     */
    suspend fun parse(args: Array<String>): ArgParserResult = parse(args.asList())

    protected suspend fun parse(args: List<String>): ArgParserResult {
    ...
    fun <T> withData(callback: (options: Map<String, ParsingValue<*, *>>, arguments: Map<String, ParsingValue<*, *>>, subcommands: Map<String, Subcommand>) -> T): T {
        return callback(options, arguments, subcommands)
    }
...
fun ArgParser.getHelpEmbed(): EmbedBuilder = this.withData { options, args, subs ->
    EmbedBuilder().apply {
        title = programName
        description = if (this@getHelpEmbed is Subcommand) this@getHelpEmbed.actionDescription else "Dorfbot"
        field {
            name = "subcommands"
            value = buildString {
                subs.forEach { (key, value) ->
                    appendLine(value.helpMessage)
                }
            }
        }
        field {
            ...
            value = buildString {
                options.forEach { key, value ->
                    appendLine(value.descriptor.helpMessage)
                }
            }
        }
